...
const SCHEMA = [
  {
    ...
    name: "profiles",
    attributes: [
      { key: "plan", size: 16, required: true },
      { key: "stripe_customer_id", size: 128, required: false }
    ],
    indexes: []
  },
  {
    ...
    name: "projects",
    attributes: [
      { key: "owner", size: 36, required: true },
      { key: "name", size: 128, required: true },
      { key: "slug", size: 80, required: true },
      { key: "mode", size: 16, required: true },
      { key: "source_image_path", size: 64, required: false },
      { key: "mind_path", size: 64, required: false },
      { key: "glb_path", size: 64, required: false },
      { key: "status", size: 32, required: true },
      { key: "settings", size: 10000, required: true }
    ],
    indexes: [
  ...
  },
  {
    ...
    name: "jobs",
    attributes: [
      { key: "project_id", size: 36, required: true },
      { key: "step", size: 32, required: true },
      { key: "status", size: 32, required: true },
      { key: "payload", size: 5000, required: false },
      { key: "log", size: 10000, required: false }
    ],
    indexes: [
...
async function ensureAttributes(databases, spec) {
  ...
  for (const attr of spec.attributes) {
    ...
    if (current) {
      if (current.size != null && Number(current.size) !== attr.size) {
        log(
          "warn",
    ...
    try {
      ...
      log("created", `attribute ${spec.id}.${attr.key}`);
